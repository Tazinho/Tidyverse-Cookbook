...
Note that I didn't use `drop = FALSE`, since I think it should be well known and I prefer to have a one-liner solution. I also used `[` instead of `$`, because the latter does partial matching. `iris[,"Species"]` also makes clear that we want to subset a data frame by an atomic vetor, so I didn't choose `[[` or `iris["Species"]`. In the tidyverse solution I used the pipe as I think it's general practice. For tasks about modification of atomics the solution might also be written within `mutate`.
