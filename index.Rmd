...
* When there seems to be two different base R or tidyverse approaches to a problem, just suffix these solutions with integers, eg. **TDVS1**, **TDVS2**,... .
...
* I didn't use `drop = FALSE` (in the end), since I think it should be well known and I prefer to have a one-liner solution. 
* I also used `[` instead of `$`, because the first doesn't use partial matching. The solution via brackets makes it also really easy to turn the code into a function like `function(dat, var){dat[var, ]}`
...
However, these are just details and the structure might change anyway, so when in doubt just push your edits and I`ll try to take care on the remain.
